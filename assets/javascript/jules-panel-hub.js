...
window.fetchHubPRs = async function(owner, repo) {
    ...
    window.JulesPanelState.unreadPRs = prs.length;
    updateHubBadges();
}
...
window.fetchHubIssues = async function(owner, repo) {
    ...
    window.JulesPanelState.unreadIssues = issues.length;
    updateHubBadges();
}
...
window.fetchHubNotifs = async function() {
    ...
    window.JulesPanelState.unreadNotifs = notifs.length;
    updateHubBadges();
}

window.updateHubBadges = function() {
    const prs = window.JulesPanelState.unreadPRs || 0;
