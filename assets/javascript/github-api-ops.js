...
// Used by: dashboard.html (new Tailwind OPS dashboard)
// DO NOT load this file on dashboard.md — use github-api.js instead
// ============================================================
...
async function atomicWrite(filePath, mutatorFn, commitMessage, mergeStrategyFn) {
    ...
    while (attempt < MAX_RETRIES) {
        ...
        try {
            ...
            if (result.status === 409) {
                console.warn(`[AtomicWrite] Conflicto en ${filePath} (intento ${attempt}/${MAX_RETRIES}). Ejecutando merge...`);
                const { content: freshRemote, sha: freshSha } = await fetchFileWithSha(filePath);
                const merged = mergeStrategyFn
                    ? mergeStrategyFn(newContent, freshRemote)
                    : { ...freshRemote, ...newContent };
                const retryResult = await putFileContent(
                    filePath, freshSha, merged,
                    `${commitMessage} [merge-retry-${attempt}]`
                );
                if (retryResult.ok) {
                    if (filePath.includes('dashboard_tasks')) {
            ...
            throw new Error(`Error inesperado escribiendo ${filePath}: HTTP ${result.status}`);
    ...
    throw new Error(`[AtomicWrite] Fallo permanente en ${filePath} tras ${MAX_RETRIES} intentos. Último error: ${lastError}`);
}
...
function computeAllMemberStats(tasks, members) {
    ...
    for (const member of members) {
        ...
        result[member]  = {
            found, fixed, fixed_ok: fixedOK, support, support_ok: supportOK,
...
function computeHallOfFame(tasks, members) {
    ...
    return Object.entries(stats)
        ...
        .map(([name, s], i) => ({
            rank: i + 1, medal: MEDALS[i] || '', name, score: s.score,
...
function computeBurnoutIndex(tasks, currentMilestoneId, milestoneStartDate, milestoneEndDate) {
    ...
    const milTasks    = tasks.filter(t =>
        t.milestone === currentMilestoneId &&
        t.estado !== 'Obsolete' &&
        t.estado !== 'OK'
    );
...
console.log('[github-api-ops.js] Motor atómico OPS cargado. window.githubOpsApi expuesto.');
