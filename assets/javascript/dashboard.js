...
function buildTaskCard(task) {
  ...
  const getAvatar = (name) => {
    ...
    const profileKey = currentProfiles?.members ? Object.keys(currentProfiles.members).find(k => k.toLowerCase() === name.toLowerCase()) : null;
    const profile = profileKey ? currentProfiles.members[profileKey] : null;

    if (profile && profile.handle) {
      return `<img src="https://github.com/${profile.handle}.png" alt="${name}" class="w-full h-full object-cover">`;
    }

    // Fallback: if name looks like a handle, try it directly
    if (name.match(/^[a-z0-9-]{3,}$/i)) {
      return `<img src="https://github.com/${name}.png" alt="${name}" class="w-full h-full object-cover">`;
    }
    ...
    <div class="flex justify-between items-end">
      <div class="flex flex-col gap-2">
        ...
        <div class="flex -space-x-2">
          ${task.resuelto_por ? `<div onclick="scrollToProfile('${task.resuelto_por}')" title="Resuelto por: ${task.resuelto_por}" class="w-6 h-6 rounded-full bg-emerald-500 border-2 border-slate-800 flex items-center justify-center text-[8px] font-bold text-slate-950 cursor-pointer hover:scale-110 transition-transform overflow-hidden shadow-md">${getAvatar(task.resuelto_por)}</div>` : ''}
          ${task.detectado_por ? `<div onclick="scrollToProfile('${task.detectado_por}')" title="Detectado por: ${task.detectado_por}" class="w-6 h-6 rounded-full bg-indigo-500 border-2 border-slate-800 flex items-center justify-center text-[8px] font-bold text-white cursor-pointer hover:scale-110 transition-transform overflow-hidden shadow-md">${getAvatar(task.detectado_por)}</div>` : ''}
          ${task.apoyo ? `<div onclick="scrollToProfile('${task.apoyo}')" title="Apoyo: ${task.apoyo}" class="w-6 h-6 rounded-full bg-purple-500 border-2 border-slate-800 flex items-center justify-center text-[8px] font-bold text-white cursor-pointer hover:scale-110 transition-transform overflow-hidden shadow-md">${getAvatar(task.apoyo)}</div>` : ''}
        </div>

        <!-- Assigned Team -->
        <div onclick="openAssignmentModal(${task.id})" class="flex items-center gap-1.5 cursor-pointer hover:opacity-80 transition-opacity min-h-[24px]">
          <div class="flex -space-x-1.5">
            ${(task.asignados || []).length > 0
              ? task.asignados.map(handle => {
                  const name = MEMBER_MAPPING[handle] || handle;
                  return `<img src="https://github.com/${handle}.png" class="w-6 h-6 rounded-full border-2 border-slate-800 shadow-sm bg-slate-900" title="Asignado: ${name}">`;
                }).join('')
          ...
          <span class="text-[8px] font-bold text-slate-500 uppercase tracking-tighter">Asignados</span>
        </div>
      </div>
